...
SHOW_PREVIEW = True  # 強制顯示 segmentation 視窗
...
class CarEnv(gym.Env):
	...
	def step(self, action):
		...
		# --- Third-person spectator camera follow logic ---
		trans = self.vehicle.get_transform()
		# 計算車輛後方 10 公尺、上方 5 公尺的位置
		back_vec = carla.Location(x=-10 * math.cos(math.radians(trans.rotation.yaw)),
								  y=-10 * math.sin(math.radians(trans.rotation.yaw)),
								  z=5)
		cam_location = trans.location + back_vec
		# 鏡頭朝向車輛
		cam_rotation = carla.Rotation(pitch=-15, yaw=trans.rotation.yaw)
		self.spectator.set_transform(carla.Transform(cam_location, cam_rotation))
		# --- End spectator follow ---
		...
		# showing image
		cv2.imshow('Carla Segmentation View', cam)
		cv2.waitKey(1)
	...
	def reset(self):
		...
		trans = self.vehicle.get_transform()
		spectator_pos = carla.Transform(trans.location + carla.Location(x=0, y=0, z=50),
